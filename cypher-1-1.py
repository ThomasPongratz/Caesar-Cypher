...
def print_translated(translated):
    ...
    lbl = tk.Label(
        ...
        font=("Fira Code", 9), bg='#FAF8CC', fg='black'
        )
    ...
    text_box.grid(row=5, column=1, columnspan=2, pady=2, sticky="w")
    text_scroll.grid(row=5, column=2, pady=2, sticky="nse")
...
def caesar(input_mode):
    ...
    message = input_textbox.get("1.0",'end-1c')
...
root.config(bg='#FAF8CC')

# HEADING
heading = tk.Label(
    root, text="Caesar-Cypher", 
    font=("unispace", 12), bg='#FAF8CC', fg='black'
    )
heading.grid(row=0, column=0, columnspan=3)

heading2 = tk.Label(
    root, text="...encrypt and decrypt messages", 
    font=("unispace", 9), bg='#FAF8CC', fg='black'
    )
heading2.grid(row=1, column=0, columnspan=3)
...
input_label.grid(row=2, column=0, columnspan=1)
...
text_scroll.config(command=input_textbox.yview, )
input_textbox["yscrollcommand"] = text_scroll.set

# input_textbox.get('end', translated)
# input_textbox.config(state='disabled')

input_textbox.grid(row=2, column=1, columnspan=2, sticky="w")
text_scroll.grid(row=2, column=2, sticky="nse")
...
key_label.grid(row=3, column=0, columnspan=1)

key_entry = tk.Entry(root)
key_entry.grid(row=3, column=1, columnspan=2)
...
encrypt_But.grid(row=4, column=1, padx=5, pady=5)
...
decrypt_But.grid(row=4, column=2, pady=5)
